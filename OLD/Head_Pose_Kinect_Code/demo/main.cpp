...
bool process() {
	...
	//cout << g_means.size() << endl;
	if(g_means.size()>0)
	{
		cout << "POS--(X,Y,Z):   " << g_means[0].val[0] << "  " << g_means[0].val[1] << "  " << g_means[0].val[2] << endl;
		cout << "ANG--(P,Y,R):   " << g_means[0].val[3] << "  " << g_means[0].val[4] << "  " << g_means[0].val[5] << endl << endl; 
	}
	//if(g_means.size()>0)
